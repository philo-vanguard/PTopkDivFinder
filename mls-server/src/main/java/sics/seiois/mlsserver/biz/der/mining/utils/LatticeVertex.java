...
public class LatticeVertex implements KryoSerializable {
    ...
    private int max_predicate_index_in_X = -1;

    public void setMax_predicate_index_in_X(int max_pred_index_in_X) {
        this.max_predicate_index_in_X = max_pred_index_in_X;
    }

    public int getMax_predicate_index_in_X() {
        return this.max_predicate_index_in_X;
    }
    ...
    public LatticeVertex(Predicate p, int maxTupleID) {
        ...
        this.max_predicate_index_in_X = Math.max(this.max_predicate_index_in_X, PredicateSet.getIndex(p));
    }

    public LatticeVertex(PredicateSet Xset, int maxTupleID) {
        this.predicates = Xset;
        this.rhss = new PredicateSet();
        this.maxTupleID = maxTupleID;
        this.relations = new String[this.maxTupleID + 1];
        for (int i = 0; i <= this.maxTupleID; i++) {
    ...
    public LatticeVertex(LatticeVertex parents, Predicate p) {
        ...
        // update max predicate index in X
        this.max_predicate_index_in_X = Math.max(parents.getMax_predicate_index_in_X(), PredicateSet.getIndex(p));
    }
    ...
    public void addRHSs(PredicateSet rhss) {
//        for (Predicate p : rhss) {
//            this.rhss.add(p);
//        }
...
//            }
    ...
    /*
        ...
        Example:
            Given a LatticeVertex such as "p1 p2 p3", we remove a predicate as the RHS, and let the rest as X.
            Thus, for this LatticeVertex, we get three work units: p1 p2 -> p3; p1 p3 -> p2; p2 p3 -> p1.
        Thus, when level = 0, the LatticeVertex only contains one predicate, and return empty work units.
     */
...
//        kryo.writeObject(output, interestingnessUB);
        kryo.writeObject(output, max_predicate_index_in_X);
...
//        this.interestingnessUB = kryo.readObject(input, HashMap.class);
        this.max_predicate_index_in_X = kryo.readObject(input, Integer.class);
    }
