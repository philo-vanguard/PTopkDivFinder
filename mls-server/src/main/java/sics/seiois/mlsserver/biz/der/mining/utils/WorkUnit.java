...
//        }
    ...
    public WorkUnit(WorkUnit task) {
        this.currrent = new PredicateSet(task.getCurrrent());
        this.rhss = new PredicateSet(task.getRHSs());
        this.currrentIndexs = new ArrayList<>(task.getCurrentIndexs());
        this.rhsIndexs = new ArrayList<>(task.getRHSIndexs());
        this.allCount = task.getAllCount();
        this.columnsValueInt = new HashMap<>(task.getColumnsValueInt());
        this.columnsPLI = new HashMap<>(task.getColumnsPLI());
        this.tidsIntervals = new HashMap<>(task.getTidsIntervals());
        this.predicatesMLData = new HashMap<>(task.getPredicatesMLData());
        this.minPredicateSupportT0T1 = task.getMinPredicateSupportT0T1();
        if (task.getPids() == null) {
            this.pids = null;
        } else {
            this.pids = new int[task.getPids().length];
            for (int i = 0; i < task.getPids().length; i++) {
                this.pids[i] = task.getPids()[i];
            }
        }
    }
...
//        log.info("### Work unit PLI : {}", this.columnsPLI);
    ...
    public void addCurrent(PredicateSet ps) {
        this.currrent.addAll(ps);
    }
    ...
    public void addRHS(PredicateSet ps) {
        this.rhss.addAll(ps);
    }
